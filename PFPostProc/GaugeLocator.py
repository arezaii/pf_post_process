...
def find_pftcl_file(pf_outputs):
    ...
    if len(pftcl_file) != 1:
        print(f'incorrect number of pftcl files found!\nExpected: 1, Got: {len(pftcl_file)}')
        exit(1)
...
def find_domain_extents(pf_outputs, runname):
    ...
    if len(extents_file) != 1:
        print(f'incorrect number of extents files found!\nExpected: 1, Got: {len(extents_file)}')
        exit(1)
...
def generate_flow_at_gauges(pf_outputs, out_dir, start_date=None):
    ...
    gauges_in_extents = gauges_in_extents.assign(mapped_j=lambda row: row.Final_j - conus_lower_left[1])
    ...
    gauges_in_extents = gauges_in_extents.assign(get_data=lambda row: mask_data[0, row.mapped_j, row.mapped_i])
    ...
    if not gauges_in_mask.empty:
        ...
        gauges_in_mask = gauges_in_mask.assign(slope_y=lambda row: slope_data_y[0, row.mapped_j, row.mapped_i])
        gauges_in_mask = gauges_in_mask.assign(slope=lambda row: (row.slope_x ** 2 + row.slope_y ** 2) ** 0.5)
        ...
        for p_file in pressure_files:
            ...
            p_data = p_data.assign(timestep=ts)
        ...
        flow_data = flow_data.assign(flow_cfs=lambda row: row.flow_cms * CMS_TO_CFS)
        ...
        for name, group in flow_data.filter(['STAID', 'STANAME', 'timestep', 'flow_cms', 'flow_cfs', 'mapped_i','mapped_j']).groupby('STAID'):
